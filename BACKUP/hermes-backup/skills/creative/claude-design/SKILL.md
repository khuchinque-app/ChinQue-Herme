...
metadata:
  hermes:
    ...
    related_skills: [design-md, popular-web-designs, excalidraw, architecture-diagram, design-research]
---
...
## Design Research — Extracting Context from References

**When the user shares a design reference URL** (Figma community, Dribbble, Behance) or a screenshot as input, and you cannot directly inspect it visually, use this systematic fallback.

### Core problem
You may operate without vision tools or browser automation. A user sharing a design link expects you to see and understand it. The fallback: extract metadata, download what you can, search alternatives, and — when all else fails — ask the right questions.

### Workflow

**1. Fetch the source page metadata** via `web_extract(urls=["<url>"])`:
- Title, description, tags/comments, cover/preview image URL
- For Figma Community: title is rich metadata, comments reveal polish level
- For Dribbble: many old shots return 404 — search by title
- For Behance: generally returns more metadata than Dribbble

**2. Attempt image download** via `terminal`:
```bash
curl -s -o /tmp/reference.png "<image-url>" && file /tmp/reference.png
```
Result `PNG/JPG image data` = success. Result `HTML document` = CDN blocked.

**3. Search alternative sources:** figma-free.com, Google Images, Pinterest

**4. When you must ask,** prioritize: surface type (mobile/dashboard/landing), key screens/flows, color palette, layout pattern, visual density, typography direction.

### Known gotchas
- **Figma S3 (s3-alpha.figma.com) ALWAYS blocks** — CloudFront 403. Don't retry.
- **Dribbble aggressively blocks scraping** — limited metadata from curl/web_extract.
- Do NOT fabricate visual details from a reference you couldn't inspect.
- Do NOT spend more than 3-4 tool calls trying to retrieve a single image.

### Full reference files
- `skill_view(name="claude-design", file_path="references/live-css-deployment.md")` — Replacing styles on existing PHP projects
- `skill_view(name="claude-design", file_path="references/figma-food-delivery-session.md")` — Figma URL extraction worked example
