...
Browse, install, and select animated "pet" mascots from the public petdex gallery. An installed pet reacts to agent activity (idle, running a tool, reviewing, error, done) across the Hermes CLI, TUI, and desktop app.

### Commands

| Goal | Command |
| --- | --- |
| Browse gallery | `hermes pets list` (add substring to filter: `hermes pets list cat`) |
| Installed pets | `hermes pets list --installed` |
| Install + activate | `hermes pets install <slug> --select` |
| Set active | `hermes pets select <slug>` |
| Change size | `hermes pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview | `hermes pets show [slug] [--cycle] [--state run]` |
| Disable | `hermes pets off` |
| Remove | `hermes pets remove <slug>` |
...
- `scale` (float) — on-screen size of native 192×208 frames (default 0.33, clamped 0.1–3.0)
- `unicode_cols` (int) — width in columns for the Unicode fallback

### Pitfalls

- Pets install into `<HERMES_HOME>/pets/<slug>/` (profile-aware, not `~/.codex/pets`).
- A pet only shows once installed AND selected (`enabled: true`) — `hermes pets doctor` confirms readiness.
- Inside a pipe/redirect (no TTY) terminal rendering is disabled by design.
- For full-fidelity rendering: kitty, Ghostty, WezTerm, iTerm2, or sixel-capable terminal. Otherwise truecolor Unicode half-block fallback is used automatically.
...
## Skill Authoring Guide (agent-created skills)

When the user-local agent (`skill_manage action=create`) writes a new skill to `~/.hermes/skills/`, it must follow these rules.

### Required Frontmatter

- Starts with `---` as first bytes (no leading blank line).
- Closes with `\n---\n` before the body.
- `name` field present (max 64 chars, lowercase + hyphens).
- `description` field present, ≤ 1024 chars.
- Non-empty body after closing `---`.

Peer-matched shape (the consensus across the 56 current skills):
...
1. **Optimize for process predictability.** Every line should change behavior vs the default. If a line doesn't, cut it.
2. **Choose the right context load.** The description pays for itself every turn — keep descriptions focused on trigger classes.
3. **Use an information hierarchy.** Always-needed steps in SKILL.md body; bulky/branch-specific detail in `references/`, `templates/`, or `scripts/` loaded on demand.
4. **End steps with completion criteria.** Each ordered step should say how the agent knows it is done.
5. **Co-locate rules with the concept they govern.** Keep definition, caveats, examples, and verification near each other.
6. **Use strong leading words** the model already knows — "tight loop," "tracer bullet," "root cause."
7. **Prune duplication and no-ops.** Sentence by sentence: does this change agent behavior? If not, delete it.

### Common Quality Failures

| Failure | Fix |
|---------|-----|
| **Premature completion** — skill lets agent move on before work is done | Sharpen completion criteria |
| **Duplication** — same rule in multiple places | Consolidate to one source |
| **Sediment** — stale lines accumulate | Remove old wording when adding new |
| **Sprawl** — too much always-visible material | Push branch-specific detail behind pointers |
| **No-op prose** — generic advice agent would follow anyway | Replace with checkable criterion |

### Size Constraints

- Description: ≤ 1024 chars
- Full SKILL.md: ≤ 100,000 chars (aim for 8-14k)
- Push past 20k → split into `references/*.md`

### Cross-Referencing

`metadata.hermes.related_skills` unions both in-repo (`skills/`) and user-local (`~/.hermes/skills/`) trees at load time. Prefer referencing only broad class-level skills. Never reference session-narrow skills — they'll be archived.

### Verification Checklist

- [ ] File at correct category path
...
- [ ] No-op prose, sediment, and duplication removed
