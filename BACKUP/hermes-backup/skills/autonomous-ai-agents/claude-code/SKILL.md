...
## Related Coding Agent Tools

Hermes can also delegate to other autonomous coding CLIs. These follow the same
orchestration pattern but have different CLI names and flags.

### OpenAI Codex CLI

Codex is OpenAI's coding agent. Use `codex exec "prompt"` for one-shot tasks.

**Key differences from Claude Code:**
- Requires `pty=true` in all terminal calls — Codex is interactive-only
- Needs a git repository to run; use `mktemp -d && git init` for scratch work
- Flags: `--full-auto` (sandboxed auto-approve), `--yolo` (no sandbox)
- Use `process` tools to monitor background Codex sessions
- Hermes gateway caveat: use `--sandbox danger-full-access` when bubblewrap fails

**Full reference:** `skill_view(name="claude-code", file_path="references/codex.md")`

### OpenCode CLI

OpenCode is a provider-agnostic open-source coding agent. Use `opencode run 'prompt'`
for one-shot tasks (no pty needed) or `opencode` with `background=true, pty=true`
for interactive sessions.

**Key differences from Claude Code:**
- `opencode run` does NOT need pty — only the TUI mode does
- Do NOT use `/exit` — use Ctrl+C (`\\x03`) or `process(action="kill")`
- Supports `--model provider/model` to force a specific model
- Supports `--thinking` to show model reasoning
- Supports `--continue` / `-c` to resume the last session

**Full reference:** `skill_view(name="claude-code", file_path="references/opencode.md")`

### Choosing a coding agent

| Criterion | Claude Code | Codex | OpenCode |
|-----------|------------|-------|----------|
| One-shot tasks | `-p` flag (preferred) | `exec "prompt"` | `run "prompt"` |
| Interactive sessions | tmux-based | `pty=true` | `pty=true` |
| Provider | Anthropic only | OpenAI only | Any (OpenRouter, etc.) |
| Needs git repo | No | Yes | Recommended |
| Model selection | `--model` flag | N/A | `--model provider/model` |
