...
## Section: Sketch — Throwaway HTML Mockups

Use when the user wants to see a design direction before committing — exploring a UI/UX idea as 2-3 disposable HTML variants for side-by-side comparison. The point is comparison, not production code.

**When NOT to use:** User wants production components (use claude-design main workflow), a diagram (use architecture-diagram), or designs are already locked.

### Core Method

1. **Intake** (if user hasn't given enough): ask about the *feel*, *references*, and *core action* — one question at a time
2. **Variants:** Produce **2-3** in one go, each a standalone HTML file. Each takes a **different design stance**, not different pixel values:
   - Density: compact / airy / ultra-dense
   - Emphasis: content-first / action-first / tool-first
   - Layout: single-column / sidebar / split-pane
3. **Variant naming:** describe the stance, not the number (`001-calm-editorial/`, `001-utilitarian-dense/`)
4. **Build real HTML:** Inline `<style>`, system fonts or one Google Font, CDN Tailwind OK, realistic fake content (not lorem ipsum), interactive (links, hovers, one state transition)
5. **Verify visually** using Hermes browser tools — load each variant and inspect with `browser_vision`
6. **Head-to-head comparison table** presenting variants with opinionated assessment
...
### Frontier Mode
When existing sketches exist and user says "what should I sketch next", look for: consistency gaps between winning variants, unsketched screens, state coverage gaps (empty/loading/error), responsive gaps, interaction patterns.

## Section: DESIGN.md — Design Token Spec Files

For authoring formal, machine-readable design system spec files following Google's open DESIGN.md spec. One file combines YAML frontmatter (machine-readable tokens) + Markdown body (human-readable rationale).

### When to Use

- User asks for a DESIGN.md file, design tokens, or design system spec
- User wants consistent UI/brand across multiple projects
- User wants WCAG contrast validation on their color palette
- The deliverable is a *spec file*, not a rendered artifact
...
1. **Author** `DESIGN.md` in project root via `write_file`
2. **Lint** with CLI: `npx -y @google/design.md lint DESIGN.md` (checks structure + token references + WCAG contrast)
3. **Diff** versions: `npx -y @google/design.md diff DESIGN.md DESIGN-v2.md`
4. **Export** to Tailwind: `npx -y @google/design.md export --format tailwind DESIGN.md > tailwind.theme.json`
5. **Export** to W3C DTCG: `npx -y @google/design.md export --format dtcg DESIGN.md > tokens.json`

### Key Rules
- Token references use dotted paths (`{colors.primary}`, not `{primary}`)
- Component variants are sibling keys, not nested (`button-primary-hover`, not `button-primary.hover`)
...
- Token references resolve by dotted path only
