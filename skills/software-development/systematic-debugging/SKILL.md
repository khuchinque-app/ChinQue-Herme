...
## Section: Python Debugging (pdb + debugpy)

When you need to inspect Python code state — three tools, picked by situation:

| Tool | When |
|---|---|
| **`breakpoint()` + pdb** | Local, interactive, simplest. Add `breakpoint()`, run normally. |
| **`python -m pdb`** | Launch a script under pdb with no source edits. |
| **`debugpy`** | Remote / headless / attach to running process (gateway, daemon, PTY children). |

### pdb Quick Reference (inside `(Pdb)` prompt)
...
**Local breakpoint:** Add `breakpoint()` in source, run code. Remove before committing (`rg -n 'breakpoint\\(\\)' --type py`).

**Pytest under debugger:** `scripts/run_tests.sh tests/test_file.py::test_name --pdb -p no:xdist` (pdb requires `-p no:xdist` to disable parallel workers).
...
```bash
# Launch: add near entry point
import debugpy
debugpy.listen(("127.0.0.1", 5678))
debugpy.wait_for_client()
```

**Remote attach with remote-pdb (simpler, terminal-friendly):**
```python
from remote_pdb import set_trace
set_trace(host="127.0.0.1", port=4444)  # blocks; connect with `nc 127.0.0.1 4444`
```

### Hermes-Specific Tips
- Debugging `tui_gateway` subprocess: use `remote-pdb` at the specific RPC handler
- Debugging `_SlashWorker`: same pattern — `set_trace()` inside the worker's exec path
- Gateway (`gateway/run.py`): use `remote-pdb` at a handler or `debugpy` with `--wait-for-client`
- pdb under pytest-xdist hangs silently — always use `-p no:xdist` or `-n 0`
- `PYTHONBREAKPOINT=0` disables all `breakpoint()` calls

## Section: Node.js Debugging (node inspect + CDP)

When `console.log` isn't enough. Two tools: **`node inspect`** (built-in, zero-install CLI) and **CDP via `chrome-remote-interface`** (scriptable, automatable).
...
- **"Why is this variable undefined?"**
  ...
  node inspect -p $!
  # debug> sb('script.js', 42); cont; repl; > myVariable
  ```
- **"What's the call path?"** `debug> sb('suspectFn'); cont; bt`
- **"Async chain hangs?"** Start with `--inspect` (no `-brk`), let it hang, `debug> pause; bt`

### CDP Automation

For scriptable breakpoints and scope inspection (needs `npm i -g chrome-remote-interface`):
```javascript
const CDP = require('chrome-remote-interface');
const client = await CDP({ port: 9229 });
const { Debugger, Runtime } = client;
// See node-inspect-debugger skill for full driver script
```

### Pitfalls
- **Wrong line numbers in TS:** breakpoints hit emitted JS, not `.ts`. Use `dist/*.js` or CDP clients with sourcemap support.
- **Port collisions:** Default is `9229`. Use `--inspect=0` for random port, list with `curl http://127.0.0.1:9229/json/list`.
- **Child processes:** Use `NODE_OPTIONS='--inspect-brk' node parent.js` to propagate.
- Always bind to `127.0.0.1` unless on isolated network.
